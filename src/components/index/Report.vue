    ...
    <div>
        <el-card class="box-card" style="height: 650px;">
            <el-collapse v-model="activeName" accordion>
                <el-collapse-item v-for="(value, key) in flieList" :name="key">
                    <template #title>
                        <p class="title">{{ key }}</p>
                    </template>

                    <el-row :gutter="10">
                        <el-col :xs="8" :sm="6" :md="4" style="padding-top: 10px;" v-for="fileName in value">
                            <el-card class="flie-box-card">
                                <template #header>
                                    <div class="card-header">
                                        <span>{{ fileName.substring(3, 11) }}</span>

                                    </div>
                                </template>
                                <div style="text-align: center;">
                                    <svg t="1705276917370" class="icon" viewBox="0 0 1024 1024" version="1.1"
                                        xmlns="http://www.w3.org/2000/svg" p-id="1665" width="40" height="40">
                                        <path
...
const getFileList = () => {
    get('/heiMaoSub/fileList', (message) => {

        let years = {};
        message.forEach(file => {
            let year = file.substring(3, 7); // 提取年份
            if (!years[year]) {
                years[year] = []; // 如果该年份还没有被初始化，则初始化为一个空数组
                if (year > activeName.value) {
                    activeName.value = year
                }
            }
            years[year].push(file); // 将文件添加到对应年份的数组中
        });

        flieList.value = years;
